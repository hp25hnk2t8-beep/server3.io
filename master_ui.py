...
# ================= PIN ԿԱՐԳԱՎՈՐՈՒՄ =================
MASTER_PIN = "1973"
DASHBOARD_PIN = "0000"  # NEW: Dashboard PIN
MOBILE_PIN = "1111"     # NEW: Mobile PIN
# ====================================================
...
<div id="dashboard" class="dashboard">
    ...
    <div class="results-card">
        ...
        <div class="table-container"><table id="dashTable"><thead><tr><th onclick="sortBy('status')">Status</th><th onclick="sortBy('username')">Username</th><th onclick="sortBy('balance')">Balance</th></tr></thead><tbody id="dashTableBody"><tr><td colspan="3" style="text-align:center; padding:30px;">Loading...</td></tr></tbody></table></div>
    </div>
...
function renderTable(){let filtered=dashResults;let search=document.getElementById('dashSearch')?.value.toLowerCase()||'';if(search)filtered=filtered.filter(r=>r.username.toLowerCase().includes(search));filtered.sort((a,b)=>{let av=currentSort.field==='balance'?(a.balance_value||0):(a[currentSort.field]||'').toString().toLowerCase();let bv=currentSort.field==='balance'?(b.balance_value||0):(b[currentSort.field]||'').toString().toLowerCase();if(typeof av==='number')return currentSort.dir==='asc'?av-bv:bv-av;return currentSort.dir==='asc'?(av>bv?1:-1):(av<bv?1:-1);});let balanceClass=(v)=>{let n=parseFloat(v)||0;return n>100?'balance-positive':n>10?'balance-medium':'balance-zero';};document.getElementById('dashTableBody').innerHTML=filtered.map(r=>`<tr><td style="font-size:16px">${r.status}</td><td><strong style="color:#58a6ff">${escapeHtml(r.username)}</strong><button class="copy-btn" onclick="copyToClipboard('${escapeHtml(r.username)}')"><i class="fas fa-copy"></i></button></td><td class="${balanceClass(r.balance_value)}">${r.balance||'0 ֏'}</td></tr>`).join('');}
function updateStats(){document.getElementById('dashTotal').innerText=dashResults.length;document.getElementById('dashSuccess').innerText=dashResults.filter(r=>r.status==='✅').length;document.getElementById('dashFailed').innerText=dashResults.filter(r=>r.status==='❌').length;document.getElementById('dashTimeout').innerText=dashResults.filter(r=>r.status==='⏰').length;}
...
<div id="mainContent" class="main-content"><div class="container"><div class="header"><h1><i class="fas fa-network-wired"></i> MASTER UI | Multi Bot Aggregator</h1><div class="header-sub">📡 Արդյունքները ՊԱՀՊԱՆՎՈՒՄ ԵՆ | Թարմացվում է միայն բալանսը</div></div>
<div class="stats-top"><div class="stat-card" onclick="setFilter('all')"><div class="stat-number" id="totalCount">0</div><div class="stat-label">TOTAL</div></div><div class="stat-card" onclick="setFilter('success')"><div class="stat-number" id="successCount">0</div><div class="stat-label">✅ SUCCESS</div></div><div class="stat-card" onclick="setFilter('failed')"><div class="stat-number" id="failedCount">0</div><div class="stat-label">❌ FAILED</div></div><div class="stat-card" onclick="setFilter('timeout')"><div class="stat-number" id="timeoutCount">0</div><div class="stat-label">⏰ TIMEOUT</div></div></div>
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("📌 FEATURES:")
    print("   ✅ Persistent results - never lost on new cycles")
    print("   ✅ Dashboard - compact view (stats + table)")
    print("   ✅ Mobile - card view optimized for phones")
    print("   ✅ Clear All Results + Delete Row")
    print("=" * 60 + "\n")
