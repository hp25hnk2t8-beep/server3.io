...
# ================= LIVE USERS COUNTER =================
active_users: Dict[str, float] = {}  # session_id -> last_activity

def update_user_activity(session_id: str):
    active_users[session_id] = time.time()

def get_active_users_count(timeout_seconds: int = 60) -> int:
    now = time.time()
    expired = [sid for sid, ts in active_users.items() if now - ts > timeout_seconds]
    for sid in expired:
...
# ================= TELEGRAM SENDER (OPTIMIZED) =================
_last_telegram_report_time = None
_telegram_report_lock = asyncio.Lock()
...
async def fetch_and_merge_results() -> List[Dict]:
    ...
    # Telegram report (every 10 minutes, non-blocking)
    global _last_telegram_report_time
    now = datetime.now()
    if _last_telegram_report_time is None or (now - _last_telegram_report_time).seconds > 600:
        async with _telegram_report_lock:
            if _last_telegram_report_time is None or (now - _last_telegram_report_time).seconds > 600:
                timeout_accounts = []
                failed_accounts = []
                for username, acc in cached_results.items():
                    ...
                    if status == "⏰":
                        timeout_accounts.append(f"• {username} - {acc.get('error', 'Timeout')[:50]}")
                    elif status == "❌":
                        failed_accounts.append(f"• {username} - {acc.get('error', 'Failed')[:50]}")
                
                if timeout_accounts or failed_accounts:
                    msg = "<b>🚨 MASTER UI - Problem Accounts</b>\n\n"
                    if timeout_accounts:
                        msg += f"<b>⏰ Timeout ({len(timeout_accounts)}):</b>\n" + "\n".join(timeout_accounts[:15]) + "\n\n"
                    if failed_accounts:
                        msg += f"<b>❌ Failed ({len(failed_accounts)}):</b>\n" + "\n".join(failed_accounts[:15])
                    if len(msg) > 4000:
                        msg = msg[:4000] + "..."
                    asyncio.create_task(send_telegram_message(msg))
                    _last_telegram_report_time = now
    
    save_cached_results(cached_results)
    return list(cached_results.values())

@app.get("/results")
async def get_merged_results(request: Request):
...
async def get_live_users():
    return {"active_users": get_active_users_count()}
...
<head>
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .live-users { font-size: 10px; color: #3fb950; margin-top: 4px; }
...
<div id="dashboard" class="dashboard">
    <div class="header"><h1><i class="fas fa-chart-line"></i> Dashboard | Multi Bot Results</h1><div class="live-users" id="liveUsers">👥 Loading...</div></div>
...
let sessionId = localStorage.getItem('dashboard_session_id') || Math.random().toString(36).substring(2);
...
async function verifyPin(){let pin=document.getElementById('pinInput').value;if(!pin){document.getElementById('pinError').innerText='Enter PIN';return;}try{let res=await fetch('/dashboard/verify',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({pin:pin})});let data=await res.json();if(data.success){authToken=data.token;localStorage.setItem('dashboard_token',authToken);document.getElementById('pinOverlay').style.display='none';document.getElementById('dashboard').style.display='block';loadResults();refreshInterval=setInterval(loadResults,5000);setInterval(loadLiveUsers,10000);}else{document.getElementById('pinError').innerText='Invalid PIN';document.getElementById('pinInput').value='';}}catch(e){document.getElementById('pinError').innerText='Connection error';}}
async function loadResults(){try{let res=await fetch('/results',{headers:{'x-session-id':sessionId}});if(res.ok){dashResults=await res.json();renderTable();updateStats();}}catch(e){}}
async function loadLiveUsers(){try{let res=await fetch('/live-users');let data=await res.json();document.getElementById('liveUsers').innerHTML='👥 Online: '+data.active_users;}catch(e){}}
...
(async()=>{let savedToken=localStorage.getItem('dashboard_token');if(savedToken){try{let res=await fetch(`/dashboard/check?token=${savedToken}`);let data=await res.json();if(data.authenticated){authToken=savedToken;document.getElementById('pinOverlay').style.display='none';document.getElementById('dashboard').style.display='block';loadResults();loadLiveUsers();refreshInterval=setInterval(loadResults,5000);setInterval(loadLiveUsers,10000);}}catch(e){}}})();
</script>
</body>
</html>'''

# ================= MOBILE HTML (WITH PINS + USERS) =================
MOBILE_HTML = '''<!DOCTYPE html>
<html lang="hy">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, viewport-fit=cover, user-scalable=yes">
    <title>Mobile Monitor | Master UI</title>
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .pin-star { background: transparent; border: none; color: #d29922; cursor: pointer; font-size: 14px; margin-right: 6px; }
        .pin-star.active { color: #f0883e; }
...
<div id="mobileDashboard" class="mobile-dashboard">
    ...
    <div class="toolbar"><div><i class="fas fa-users"></i> <span id="liveUsers">-</span></div><button class="refresh-btn" onclick="loadResults()"><i class="fas fa-sync-alt"></i> Refresh</button></div>
...
let sessionId = localStorage.getItem('mobile_session_id') || Math.random().toString(36).substring(2);
...
async function verifyPin(){let pin=document.getElementById('pinInput').value;if(!pin){document.getElementById('pinError').innerText='Enter PIN';return;}try{let res=await fetch('/mobile/verify',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({pin:pin})});let data=await res.json();if(data.success){authToken=data.token;localStorage.setItem('mobile_token',authToken);document.getElementById('pinOverlay').style.display='none';document.getElementById('mobileDashboard').style.display='block';loadResults();loadLiveUsers();refreshInterval=setInterval(loadResults,5000);setInterval(loadLiveUsers,10000);}else{document.getElementById('pinError').innerText='Invalid PIN';document.getElementById('pinInput').value='';}}catch(e){document.getElementById('pinError').innerText='Connection error';}}
async function loadResults(){try{let res=await fetch('/results',{headers:{'x-session-id':sessionId}});if(res.ok){let data=await res.json();mobileResults=data;renderList();document.getElementById('lastUpdate').innerHTML='Last: '+new Date().toLocaleTimeString();}}catch(e){}}
async function loadLiveUsers(){try{let res=await fetch('/live-users');let data=await res.json();document.getElementById('liveUsers').innerText=data.active_users;}catch(e){}}
function renderList(){
    let sorted = [...mobileResults];
    sorted.sort((a,b)=>{
        let aPinned = isPinned(a.username) ? 0 : 1;
        let bPinned = isPinned(b.username) ? 0 : 1;
        if(aPinned !== bPinned) return aPinned - bPinned;
...
<head>
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .live-users { font-size: 11px; color: #3fb950; margin-top: 6px; }
...
<div id="mainContent" class="main-content"><div class="container"><div class="header"><h1><i class="fas fa-network-wired"></i> MASTER UI | Multi Bot Aggregator</h1><div class="header-sub">📡 Արդյունքները ՊԱՀՊԱՆՎՈՒՄ ԵՆ | ⭐ Pin ակաունթները միշտ վերևում</div><div class="live-users" id="liveUsers">👥 Online: --</div></div>
...
let sessionId = localStorage.getItem('master_session_id') || Math.random().toString(36).substring(2);
localStorage.setItem('master_session_id', sessionId);
...
async function initializeApp(){await loadServers();await loadResults();await updateServerStatuses();await loadLiveUsers();refreshInterval=setInterval(()=>{loadResults();updateServerStatuses();loadLiveUsers();},5000);}
async function loadLiveUsers(){try{let r=await fetch('/live-users');let d=await r.json();document.getElementById('liveUsers').innerHTML='👥 Online: '+d.active_users;document.getElementById('liveUsersFooter').innerText=d.active_users;}catch(e){}}
...
async function startServer(i){let s=currentServers[i];if(!s)return;let a=prompt(`Accounts for ${s}\n\nFormat: username:password (one per line):`,accountsText);if(!a)return;accountsText=a;addLog(`Starting ${s}...`);let btn=document.querySelector(`#serversContainer .server-item:nth-child(${i+1}) .control-start`);if(btn){btn.disabled=true;btn.innerHTML='<i class="fas fa-spinner fa-pulse"></i>';}try{let r=await fetch(`/api/control/${i}/start`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({accounts:a})});let d=await r.json();if(d.success){addLog(`Started ${s}`);setTimeout(()=>updateServerStatuses(),1000);}else{addLog(`Failed: ${d.error}`);}}catch(e){addLog(`Error: ${e.message}`);}setTimeout(()=>{if(btn){btn.disabled=false;btn.innerHTML='Start';}},3000);}
async function restartServer(i){let s=currentServers[i];if(!s)return;addLog(`Restarting ${s}...`);let btn=document.querySelector(`#serversContainer .server-item:nth-child(${i+1}) .control-restart`);if(btn){btn.disabled=true;btn.innerHTML='<i class="fas fa-spinner fa-pulse"></i>';}try{let r=await fetch(`/api/control/${i}/restart`,{method:'POST'});let d=await r.json();if(d.success){addLog(`Restarted ${s}`);setTimeout(()=>{updateServerStatuses();loadResults();},2000);}else{addLog(`Failed: ${d.error}`);}}catch(e){addLog(`Error: ${e.message}`);}setTimeout(()=>{if(btn){btn.disabled=false;btn.innerHTML='Restart';}},3000);}
async function stopServer(i){let s=currentServers[i];if(!s)return;addLog(`Stopping ${s}...`);let btn=document.querySelector(`#serversContainer .server-item:nth-child(${i+1}) .control-stop`);if(btn){btn.disabled=true;btn.innerHTML='<i class="fas fa-spinner fa-pulse"></i>';}try{let r=await fetch(`/api/control/${i}/stop`,{method:'POST'});let d=await r.json();if(d.success){addLog(`Stopped ${s}`);setTimeout(()=>updateServerStatuses(),1000);}else{addLog(`Failed: ${d.error}`);}}catch(e){addLog(`Error: ${e.message}`);}setTimeout(()=>{if(btn){btn.disabled=false;btn.innerHTML='Stop';}},3000);}
function addServer(){let i=document.getElementById('newServerInput'),v=i.value.trim();if(v){if(!v.startsWith('http://')&&!v.startsWith('https://')){addLog('Server must start with http:// or https://');return;}currentServers.push(v);i.value='';renderServersList();addLog(`Added: ${v}`);}}
function removeServer(i){let r=currentServers[i];if(confirm(`Remove ${r}?`)){currentServers.splice(i,1);renderServersList();addLog(`Removed: ${r}`);}}
async function saveServers(){let ins=document.querySelectorAll('#serversContainer input'),ups=Array.from(ins).map(i=>i.value.trim()).filter(s=>s);if(ups.length===0){addLog('No servers');return;}try{let r=await fetch('/api/servers',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({servers:ups})});if(r.ok){currentServers=ups;addLog(`Saved ${ups.length} servers`);await updateServerStatuses();}else addLog('Save failed');}catch(e){addLog(`Error: ${e.message}`);}}
async function clearAllResults(){if(confirm('Delete ALL results?')){try{let r=await fetch('/results/clear',{method:'POST'});if(r.ok){allResults=[];renderResults();updateStats();addLog('All results cleared');}}catch(e){addLog(`Error: ${e.message}`);}}}
async function deleteSingleResult(u){if(confirm(`Delete ${u}?`)){try{await fetch(`/results/clear/${encodeURIComponent(u)}`,{method:'POST'});await loadResults();addLog(`Removed ${u}`);}catch(e){addLog(`Error: ${e.message}`);}}}
async function loadResults(){try{let r=await fetch('/results',{headers:{'x-session-id':sessionId}});if(r.ok){allResults=await r.json();renderResults();updateStats();}}catch(e){addLog(`Error: ${e.message}`);}}
...
if __name__ == "__main__":
    import uvicorn
    print("\n" + "=" * 60)
    print("🖥️  MASTER UI - Multi Bot Aggregator v3.1 (LIVE USERS)")
    print("=" * 60)
    ...
    print("=" * 60)
    print("📌 NEW FEATURES:")
    print("   👥 Live Users Counter - shows how many people are online")
    print("   ⭐ Pin (Star) - click ⭐ to pin accounts on top (saved in browser)")
    print("   💰 Total Balance - shows sum of all balances")
    print("   📱 Telegram Auto-Report - every 10 min (optimized, non-blocking)")
    print("   ✅ Persistent results + fast concurrent requests")
    print("=" * 60 + "\n")
